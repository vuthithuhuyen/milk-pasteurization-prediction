...
def prepare_lstm_model(name_to_int_dict):
    model = Sequential()
    input_shape = (1, 1, len(GlobalVariables.inputfeatures))
    model.add(LSTM(16, batch_input_shape=input_shape, stateful=True))
    model.add(RepeatVector(len(name_to_int_dict)))  # Important! For connecting to time distributed layer
    model.add(TimeDistributed(Dense(len(name_to_int_dict), activation='softmax')))
